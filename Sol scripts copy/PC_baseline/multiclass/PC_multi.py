...
tree = TreeNode.read('/scratch/tianche5/PhyCollar/PC_baseline/multi_PC/tree.nwk')
...
for fold, (train_index, test_index) in enumerate(cv.split(X, y)):
    ...
    if model_choice == 'cat':
        model = cat.CatBoostClassifier(random_seed=n, loss_function='MultiClass')
        ...
        model = LinearSVC(random_state=n, multi_class='ovr')
    elif model_choice == 'lg':
        model = LogisticRegression(random_state=n, n_jobs=-1, multi_class='ovr')
    else:
        pass
    if pca_choice == 'pca':
        ...
        pass
    clf = PhyCollar(model, tree, n_jobs=-1, step_size=0.005, n_splits=1, by_clade=True, n_trials=100, n_bests=10, max_iter=200, patience=20)
    clf.fit(X_train, y_train, feature_names=features)
    y_pred = clf.predict_proba(X_test, feature_names=features)
    auc_fold = 0  # save the AUC score of each fold
    pr_fold = 0  # save the PR AUC score of each fold
    feature_maps = []
    try:
        auc_fold = roc_auc_score(y_test, y_pred, multi_class='ovr')
    except:
        auc_fold = np.nan
    try:
        pr_fold = average_precision_score(y_test, y_pred, average='macro')  # it's better to use AP instead of auc() https://scikit-learn.org/stable/modules/model_evaluation.html#precision-recall-f-measure-metrics
    except:
        pr_fold = np.nan
    try:
        fm_fold = clf.get_feature_map(features)
    except:
        fm_fold = []
    auc_df.append({'Trial': trial_index, 'Fold': fold,  'AUC': auc_fold})
    pr_df.append({'Trial': trial_index, 'Fold': fold,  'PR-AUC': pr_fold})
    fm_df.append({'Trial': trial_index, 'Fold': fold,  'FM': fm_fold})
auc_df = pd.DataFrame(auc_df)
pr_df = pd.DataFrame(pr_df)
fm_df = pd.DataFrame(fm_df)
...
fm_df.to_csv(str(output_path) + '/' + str(trial_index) + '_fm.csv', index=False)
