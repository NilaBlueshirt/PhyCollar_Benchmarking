...
# select scaler
if scaler == 'log_otu':
    # RF - Gut bowel disease paper only did log on OTU data, according to https://machinelearningmastery.com/selectively-scale-numerical-input-variables-for-machine-learning/ variables can be scaled selectively
...
for fold, (train_index, test_index) in enumerate(outer_cv.split(X, y, groups)):
    ...
    for m in range(1, 2):
        ...
        if model_choice == 'RF':
            ...
            xgb_grid.fit(X_train[:, :-1], y_train)
            # y_pred = xgb_grid.predict(pca.fit_transform(X_test[:, :-1]))
            y_pred = xgb_grid.predict(X_test[:, :-1])
            with open(output_fig_name + '_report.txt', 'a') as f:
                print('For outer cv fold ' + str(fold) + ' with random state ' + str(n), file=f)
            ...
            adab_grid.fit(X_train[:, :-1], y_train)
            warnings.filterwarnings(action="ignore", category=exceptions.FitFailedWarning)
            # y_pred = adab_grid.predict(pca.fit_transform(X_test[:, :-1]))
            y_pred = adab_grid.predict(X_test[:, :-1])
            with open(output_fig_name + '_report.txt', 'a') as f:
                print('For outer cv fold ' + str(fold) + ' with random state ' + str(n), file=f)
            ...
            # lgbm_grid.fit(pca.fit_transform(X_train[:, :-1]), y_train)
            lgbm_grid.fit(X_train[:, :-1], y_train, categorical_feature=categorical_column)
            # y_pred = lgbm_grid.predict(pca.fit_transform(X_test[:, :-1]))
            y_pred = lgbm_grid.predict(X_test[:, :-1])
            with open(output_fig_name + '_report.txt', 'a') as f:
                print('For outer cv fold ' + str(fold) + ' with random state ' + str(n), file=f)
            ...
            cat_grid.fit(X_train[:, :-1], y_train)
            # y_pred = cat_grid.predict(pca.fit_transform(X_test[:, :-1]))
            y_pred = cat_grid.predict(X_test[:, :-1])
            with open(output_fig_name + '_report.txt', 'a') as f:
                print('For outer cv fold ' + str(fold) + ' with random state ' + str(n), file=f)
            ...
            svm_grid.fit(X_train[:, :-1], y_train)
            # y_pred = svm_grid.predict(pca.fit_transform(X_test[:, :-1]))
            y_pred = svm_grid.predict(X_test[:, :-1])
            with open(output_fig_name + '_report.txt', 'a') as f:
                print('For outer cv fold ' + str(fold) + ' with random state ' + str(n), file=f)
            ...
            mlp_grid.fit(X_train[:, :-1], y_train)
            # y_pred = mlp_grid.predict(pca.fit_transform(X_test[:, :-1]))
            y_pred = mlp_grid.predict(X_test[:, :-1])
            with open(output_fig_name + '_report.txt', 'a') as f:
                print('For outer cv fold ' + str(fold) + ' with random state ' + str(n), file=f)
    ...
    max_inner_model.fit(X_train[:, :-1], y_train)  # not include the patientID at the last column of X_train
    viz = RocCurveDisplay.from_estimator(
        max_inner_model,
        # pca.fit_transform(X_test[:, :-1]),  # not include the patientID at the last column of X_test
        X_test[:, :-1],  # not include the patientID at the last column of X_test
        y_test,
